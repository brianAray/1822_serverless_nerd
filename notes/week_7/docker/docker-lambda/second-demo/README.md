...
```bash
second-demo$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
second-demo$ sam local start-api
second-demo$ curl http://localhost:3000/
```
...
```bash
second-demo$ sam logs -n HelloWorldFunction --stack-name second-demo --tail
```
...
```bash
sam delete --stack-name second-demo
```
